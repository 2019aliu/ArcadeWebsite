...
function makeAdjMat(myObj){
    ...
    var adjMat = myKeys.reduce(function(output, elem){  // creating the adjacency matrix
        // create the list of adjacencies
        var stateBorderArr = [];
        // loop through the entire list of keys, mark which ones actually border the current state
        // NOTE: FOR-EACH loop not working
        for (var i = 0; i < myKeys.length; i++) {
            nextItem = myKeys[i];
            if (myObj[elem].includes(nextItem)){
                stateBorderArr.push(true);
            }
            else {
                stateBorderArr.push(false);
            }
        }
...
function getStatesBorder(state1){
    var convertedState = processState(state1);
